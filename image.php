...
class createCapture extends Frontend
{
	...
	*/
	...
	public function __construct()
	{
		...
		session_start();
		$sk=$_GET['sk'];
		$this->zahl = strval($_SESSION['FE_DATA']["captcha_".$sk]['sum']);
		
		
		$this->c = ($GLOBALS['TL_CONFIG']['fic_length']) ? $GLOBALS['TL_CONFIG']['fic_length'] : $this->defCharCount;			
	
		//Ermitteln der Abmaße
		$this->imgWidth  = ($GLOBALS['TL_CONFIG']['fic_width']) ? $GLOBALS['TL_CONFIG']['fic_width'] : $this->defImg_width;
		$this->imgHeight = ($GLOBALS['TL_CONFIG']['fic_height']) ? $GLOBALS['TL_CONFIG']['fic_height'] : $this->defImg_height;
		
		
		// Ermitteln der Farbwerte für Hintergrund, Linien und Schrift		
		#### Hintergrundfarben######
		$this->hexBgColor   = ($GLOBALS['TL_CONFIG']['fic_bgcolor']) ? $GLOBALS['TL_CONFIG']['fic_bgcolor'] : $this->defHexBgColor;
		$this->rgbBgColor = $this-> hex2rgb($this->hexBgColor);
		
		#### Schriftfarben ######		
		$this->hexFontColor = ($GLOBALS['TL_CONFIG']['fic_fontcolor']) ? $GLOBALS['TL_CONFIG']['fic_fontcolor'] : $this->defHexFontColor;
		$this->rgbFontColor = $this-> hex2rgb($this->hexFontColor);
				
		#### Linienfarben ######
		$this->hexLineColor = ($GLOBALS['TL_CONFIG']['fic_linecolor']) ? $GLOBALS['TL_CONFIG']['fic_linecolor'] : $this->defHexLineColor;
		$this->rgbLineColor = $this-> hex2rgb($this->hexLineColor);
					
		#### Font ####			
		if($GLOBALS['TL_CONFIG']['fic_font'] && file_exists($this->defFontPath . $GLOBALS['TL_CONFIG']['fic_font']))
		{
		    
		    $this->Font = $this->defFontPath .$GLOBALS['TL_CONFIG']['fic_font'];
